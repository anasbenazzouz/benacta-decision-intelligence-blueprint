...
|---|---|---|
| Source integration | Odoo 19 JSON-2 read path verified live (36 models); the trading-company seed and the review-activity action run once for real on the owner's trial instance behind the write guard (2026-09-17) | repeat the seed and the action on a design partner's sandbox with an attested backup; a read-only bot user for ingestion; API key rotation; scheduled ingestion with monitoring; handling of Odoo record rules per company |
