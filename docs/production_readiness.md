...
|---|---|---|
| Source integration | Odoo 19 JSON-2 read path verified live (36 models); writes designed and guarded; the trading-company seed executor implemented and dry-run tested, executed only against a fake transport | run the sandbox seed (`benacta seed-odoo --confirm`) and the review-activity action with the write switch and an attested backup; a read-only bot user for ingestion; API key rotation; scheduled ingestion with monitoring; handling of Odoo record rules per company |
