...
`docs/BENACTA_Controlled_Intelligence_Blueprint.pdf` — 8–10 pages, premium LinkedIn lead magnet, useful even if the reader never opens GitHub. Designed HTML/CSS source retained at `docs/controlled-intelligence-blueprint.html`. Page plan (master brief §27): Cover → From business data to business decisions → The Decision Intelligence System → Controlled Intelligence (CODE COMPUTES / AI EXPLAINS / HUMANS DECIDE in large type) → Reference use case: Monthly Performance Review → From dashboard to decision system → Business objects before AI ("From tables to business objects") → How to start (8-step path + baseline metrics) → Reference implementation (+ placeholders: GitHub URL, live demo URL, QR) → Close ("Don't start with AI. Start with the decision." + contact placeholders).
